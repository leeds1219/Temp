...
class FLMRModelForRetrieval(FLMRPretrainedModelForRetrieval):
    ...
    def forward(
        ...
        context_pixel_values: Optional[Union[torch.Tensor, List[torch.Tensor]]] = None,
        ...
        ###########################################################################
        context_token_labels: Optional[list[int]] = None, 
        ...
        context_token_labels = context_token_labels 
        ...
        context_outputs = self.doc(
            ...
            output_hidden_states=output_hidden_states,
            ##########################
            token_labels=context_token_labels,
            ##########################
        )
    ...
    def doc(
        ...
        # TODO: add to config
        replace_vision_with_mapping = False # Ablation
        ...
        if "image" in input_modality:
            if pixel_values is not None and token_labels is None:
                # Forward the vision encoder
                pixel_values = pixel_values.to(self.device)
                vision_encoder_outputs = self.context_vision_encoder(pixel_values)
                vision_embeddings = vision_encoder_outputs.last_hidden_state[:, 0]

            elif pixel_values is not None and token_labels is not None:
                # Forward the vision encoder with optional multiple image
                all_vision_embeddings = []
                all_image_masks = []

                for image_idx, pixel_value in enumerate(pixel_values):

                    pixel_value = torch.tensor(pixel_value)           # shape: [3, 224, 224]
                    pixel_value = pixel_value.unsqueeze(0)            # shape: [1, 3, 224, 224]
                    
                    vision_encoder_outputs = self.context_vision_encoder(pixel_value)
                    vision_embeddings = vision_encoder_outputs.last_hidden_state[:, 0]
                    ...
                    spans = self.get_contiguous_spans(token_labels)  # 구간 리스트 뽑기
                    ############################################################################################
                    if self.config.use_transformer_mapping_network:
                        # select the second last layer
                        vision_second_last_layer_hidden_states = vision_encoder_outputs.hidden_states[-2][:, 1:]
                        ...
                        # cross_attention_length = self.config.transformer_mapping_cross_attention_length
                        span_start, span_end = spans[image_idx] 

                        text_encoder_hidden_states = text_encoder_hidden_states[:, span_start:span_end]
                        encoder_mask = encoder_mask[:, span_start:span_end]
                        ...
                        if replace_vision_with_mapping == False:
                            # Merge with the vision embeddings
                            vision_embeddings = torch.cat([vision_embeddings, transformer_mapping_output_features], dim=1)
                        else: 
                            vision_embeddings =transformer_mapping_output_features
        ...
        if concat_output_from_vision_encoder and concat_output_from_text_encoder and token_labels is None:
        ...
        elif concat_output_from_vision_encoder and token_labels is None:
            D = vision_embeddings
            mask = image_mask
        elif concat_output_from_text_encoder and token_labels is None:
            D = text_embeddings
            mask = mask
        ####################################################################################################################    
        elif token_labels is not None:
            assert len(token_labels) == seq_len, f"token_labels length {len(token_labels)} != seq_len {seq_len}"
            combined_embeddings = []
            combined_mask = []
            ...
            while text_idx < seq_len:
                ...
                combined_mask.append(mask[0, text_idx])  # text mask 추가
            ...
            combined_mask = torch.tensor(combined_mask).unsqueeze(0)    
...
#                 # Cross attention only attends to the first 32 tokens ##  What!!!!!
#                 encoder_mask = torch.ones_like(mask).to(mask.device, dtype=mask.dtype)
#                 if len(self.config.query_mask_input_ids_skip_list) > 0:
#                     encoder_mask[torch.isin(input_ids, torch.tensor(self.config.query_mask_input_ids_skip_list))] = 0
#                 cross_attention_length = self.config.transformer_mapping_cross_attention_length
